...
function* loadProduct({ id }: ReturnType<typeof GetProductStartAction>) {
    try {
        ...
        yield put(NotificationOpenAction('success', 'Получение продукта', 'Продукт загружен успешно'))
    }
    catch (error) {
        yield put(GetProductErrorAction(error));
        yield put(NotificationOpenAction('error', 'Получение продукта', 'Не удалось получить продукт'))
    }
}
