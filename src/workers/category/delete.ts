...
function* deleteCategory({ id }: ReturnType<typeof DeleteCategoryAction>) {
    try {
        ...
        yield put(NotificationOpenAction('success', 'Удаление категории', 'Категория удалена успешно'))
    }
    catch (error) {
        yield put(GetCategoriesErrorAction(error));
        yield put(NotificationOpenAction('error', 'Удаление категории', 'Не удалось удалить категорию'))
    }
}
